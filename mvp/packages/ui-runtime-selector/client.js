...
window.__ModuleLoader__.load({id:'@dshagent/ui-runtime-selector',factory(require){
  ...
  function RuntimeSelect({sessionId,available,locked}){
    ...
    return h('span',{style:{display:'inline-flex',alignItems:'center',gap:6,flexWrap:'wrap',fontSize:13},'data-testid':'mvp-runtime-selector'},h('label',null,'执行器 ',h('select',{'aria-label':'执行器 Runtime',value:state?.runtime||'',disabled:blocked,onChange:change,title:state?.busy?'任务执行或对账期间不能切换':'同一会话切换执行器，保留工作目录和历史',style:{font:'inherit',color:'inherit',background:'transparent',border:'1px solid #d5d9df',borderRadius:7,padding:'4px 8px'}},!state?h('option',{value:''},'加载中…'):null,(state?.runtimes||[]).map(item=>h('option',{key:item.runtimeId,value:item.runtimeId},item.label)),state&&!(state.runtimes||[]).some(item=>item.runtimeId===state.runtimeId)?h('option',{value:state.runtimeId||state.runtime},state.label||({codex:'Codex','claude-code':'Claude Code'}[state.runtime]||state.runtime)):null)),state?.busy?h('span',{role:'status'},'任务进行中，结束后可切换'):null,saving?h('span',{role:'status'},'切换中…'):null,error?h('span',{role:'alert',style:{color:'#b42318',maxWidth:360}},error):null);
  }
